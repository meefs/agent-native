...
<Diagram id="doc-block-ascsep" title={"다섯 개의 연동 레이어"} summary={"인메모리 CRDT부터 피어 간에 업데이트를 전달하는 전송까지, 각 계층에는 하나의 작업이 있습니다."}>
...
<div class="diagram-stack">
  <div class="diagram-card layer">
    ...
    ><small class="diagram-muted"
      >CRDT &mdash; 충돌 없는 병합, 조정자 없음</small
    >
  </div>
  <div class="diagram-card layer">
    <span class="diagram-pill">2 &middot; SQL 표준 콘텐츠</span
    ><small class="diagram-muted"
      >_collab_docs &mdash; durable source of truth, versioned</small
    >
  </div>
  <div class="diagram-card layer">
    <span class="diagram-pill">3 &middot; updatedAt 기준 조정</span
    ><small class="diagram-muted">에이전트 편집은 SQL bump를 통해 전파</small>
  </div>
  <div class="diagram-card layer">
    <span class="diagram-pill">4 &middot; 리드 클라이언트 선출</span
    ><small class="diagram-muted">정확히 하나의 탭이 스냅샷 적용</small>
  </div>
  <div class="diagram-card layer">
    <span class="diagram-pill ok">5 &middot; SSE 빠른 경로 + 폴링</span
    ><small class="diagram-muted"
      >수십 ms 수준, 어디서나 2초 폴링으로 폴백</small
    >
  </div>
</div>
```
...
<Diagram id="doc-block-1jdtfow" title={"두 개의 편집 경로, 하나의 병합"} summary={"인간의 키 입력 흐름은 Y.Doc → 서버 → SSE입니다. 상담원 편집은 SQL을 거칩니다. 작업 범프가 업데이트되고 리드 클라이언트가 조정된 다음 변경 사항이 Yjs에 다시 들어갑니다."}>

```html
<div class="diagram-collab">
  <div class="lane">
    <span class="diagram-pill">사람 편집</span>
    <div class="diagram-node">
      Y.Doc update<br /><small class="diagram-muted">debounce 약 80ms</small>
    </div>
  ...
  <div class="lane">
    <span class="diagram-pill warn">에이전트 편집</span>
    <div class="diagram-node">
      Action이 SQL 작성<br /><small class="diagram-muted"
        >updatedAt 업데이트</small
      >
    </div>
    <span class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</span>
    <div class="diagram-box" data-rough>
      리드 클라이언트<br /><small class="diagram-muted"
        >setContent를 Y.Doc에 적용</small
      >
    </div>
    <span class="diagram-arrow diagram-accent" aria-hidden="true">&rarr;</span>
    <div class="diagram-box diagram-accent">
...
<Callout id="doc-block-1yd00qg" tone="risk">

**동일 영역 동시 재작성은 마지막 쓰기 승리입니다.**사람이 *정확히 동일한 영역*에 저장하지 않은 편집 내용을 가지고 있는 동안 에이전트가 구절을 다시 작성하는 경우 리드 클라이언트 스냅샷이 진행 중인 사람의 편집 내용을 방해할 수 있습니다. 다른 영역의 편집 내용은 항상 CRDT를 통해 깔끔하게 병합됩니다. 구조화된 문서의 경우 세분화된 서버 측 병합을 사용하여 이를 완전히 회피합니다.
