...
<Diagram id="doc-block-1lt0gp4" title="Divulgation progressive" summary={"Seul le nom + la description de chaque compétence est toujours en contexte. Le corps entier se charge à la demande lorsque la tâche correspond."}>

```html
<div class="sk-flow">
  <div class="diagram-card">
    <span class="diagram-pill accent">Toujours dans le prompt système</span>
    <div class="sk-list">
  ...
  <div class="diagram-box">
    Full <code>SKILL.md</code> body<br /><small class="diagram-muted"
      >règles, code, à faire / à éviter</small
    >
  </div>
</div>
```
...
<AnnotatedCode
  ...
  annotations={[
    {
      lines: "2",
      label: "Clé de découverte",
      note: "The `name` matches the folder; it is how the skill is invoked as `/project-imports`.",
    },
    {
      lines: "3-5",
      label: "Le déclencheur",
      note: "Ce `description` est le **seul** texte toujours en contexte. Faites-le indiquer précisément *quand* la compétence s'applique.",
    },
    {
      lines: "9-14",
      label: "Les règles d'abord",
      note: "Commencez par la règle stricte et le pourquoi ; l’agent ne lit le corps que lorsque la tâche correspond.",
    },
    {
      lines: "27-30",
      label: "Lien croisé",
      note: "Pointez sur les compétences associées afin que l'agent puisse les enchaîner au lieu de rediriger les conseils.",
    },
  ]}
/>
...
<Diagram id="doc-block-1oobu1k" title={"Quel agent charge quelle compétence"} summary={"scope décide si l'agent d'exécution intégré à l'application voit une compétence. Les compétences dev sont visibles uniquement par votre agent de codage."}>
...
<div class="sc-grid">
  ...
  <div class="sc-targets">
    ...
    <div class="diagram-box">
      Agent de code<br /><small class="diagram-muted"
...
# Compétences Vercel/open CLI : instructions exportées uniquement, pas de configuration MCP.
npx skills@latest add BuilderIO/agent-native --skill assets

# Enregistrez un connecteur MCP hébergé pour les clients d'agent locaux.
npx @agent-native/core@latest app-skill ensure --manifest templates/assets/agent-native.app-skill.json

# Matérialisez et exécutez une source locale modifiable.
...
# Créez des adaptateurs de marché : plugin Codex, compétences Claude marketplace, Vercel,
# Compétences plain/Claude et configurations MCP.
npx @agent-native/core@latest app-skill pack --manifest templates/assets/agent-native.app-skill.json --out ./dist/assets-skill

# Installez un bundle exporté local avec les compétences Vercel/open CLI.
npx skills@latest add ./dist/assets-skill --skill assets -a codex -y

# Ajoutez la place de marché Claude Code générée, puis installez son plugin Assets.
...
# ou, sans compter sur le script du package local :
