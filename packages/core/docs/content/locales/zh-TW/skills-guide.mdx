---
title: "Skills指南"
description: "skills 如何在 Agent-Native 中工作：框架 skills、域 skills 以及建立自訂 skills。"
---
...
## skills是什麼 {#what-are-skills}

Skills 位於 `.agents/skills/<name>/SKILL.md`，包含針對代理的詳細指導。每項技能都專注於一個問題 - 如何儲存資料、如何同步狀態、如何將工作委托給代理聊天。

每個技能的 frontmatter `name` 和 `description` 總是被注入到系統提示符的 skills 塊中，以便代理知道 skills 存在什麼。當代理確定技能與工作相關時，會按需載入完整的技能主體（也透過 `docs-search` 顯示）。這就是為什麼保持描述簡短且特定於觸發器很重要：描述是代理在決定是否載入其餘部分之前讀取的唯一內容。

<Diagram id="doc-block-ywt8ns" title={"漸進式披露"} summary={"只有每個技能的名稱+描述始終處於脈絡中。當工作匹配時，全身會按需載入。"}>

```html
<div class="sk-flow">
  <div class="diagram-card">
    <span class="diagram-pill accent">始終在系統提示中</span>
    <div class="sk-list">
...
| `real-time-sync`       | 接線輪詢同步，偵錯 UI 不更新                  |
...
範本包括特定於其域的 skills。它們位於相同的 `.agents/skills/` 目錄中，但涵蓋了特定於範本的模式。請參閱每個範本的 `.agents/skills/` 目錄以取得完整清單；代表性樣本：
...
- 該指導是一次性的 - 將其放入 `AGENTS.md` 或工作區記憶中
...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      lines: "3-5",
      label: "觸發器",
      note: "這個 `description` 是始終處於脈絡中的**唯一**文字。準確地說明該技能“何時”應用。",
    },
    {
      lines: "9-14",
      label: "規則第一",
    ...
    },
    {
      lines: "27-30",
      label: "交聯",
      note: "指出相關技能，以便代理可以將它們連結起來，而不是重新得出指導。",
    },
  ]}
/>
...
當 `scope` 不存在（或設定為無法識別的值）時，它預設為 `both`，因此每個現有技能都會在執行時載入 - 該欄位完全向後相容。 `scope: dev` 技能對於任何地方的執行時代理都是不可見的：它被排除在注入系統提示符的 skills 塊和 `docs-search` 結果之外。

### 向您的編碼代理公開僅限開發的技能 {#dev-only-skills}

Agent-Native 執行時從 `.agents/skills/` 讀取 skills。 Claude 程式碼獨立從 `.claude/skills/` 讀取 skills。要使編碼代理可用但對執行時代理隱藏的技能：
...
<Diagram id="doc-block-s7y4j6" title={"哪個代理載入哪個技能"} summary={"範圍決定應用程式內執行時代理是否看到技能。開發技能僅對您的編碼代理可見。"}>
...
> **AGENTS.md** — 概述。列出所有指令碼，描述資料模型，解釋應用程式架構。代理首先閱讀此內容以了解應用程式。
>
> **Skills** — 深入研究。每項技能都側重於一種模式，並包含詳細規則、程式碼範例和“注意事項”清單。當代理需要遵循特定模式時，它會讀取這些內容。

`AGENTS.md` 告訴代理應用程式的用途。 Skills 告訴代理*如何*正確地做特定的事情。兩者都是必需的 - `AGENTS.md` 用於定向，skills 用於執行。

## Skills 與記憶 {#skills-vs-memory}

> **Skills** — 編寫的、可重複使用的操作指南。適用於每個使用者，在工作匹配時按需呼叫。
>
> **記憶 (`LEARNINGS.md` / `memory/MEMORY.md`)** — 共用專案學習和每輪都會載入的個人結構化記憶。

如果這些知識適用於在應用程式中工作的*每個人*（“總是更喜歡 CTE 而不是子查詢”），那麼它就是一項技能或共用的 `LEARNINGS.md`。如果它是關於*這個特定使用者*（“史蒂夫喜歡簡潔的答案”），它就屬於 `memory/MEMORY.md`。完整治療請參見[Workspace Memory](/docs/workspace#memory)。

---

# 進階

## 應用程式支援的 skills — 完整詳細資訊 {#app-backed-skills-full}

應用程式支援的 skills 將 Agent-Native 應用程式打包為技能市場工件。
捆綁包可以包含代理指令、匯出的 skills、MCP 連線器
中繼資料、託管/本機啟動說明和 UI 介面，例如 MCP 應用程式。

每個應用程式支援的技能都以應用程式根目錄中的 `agent-native.app-skill.json` 開頭：
...
# Vercel/open技能 CLI：僅匯出指令，無 MCP 設定。
...
# 建置市場轉接器：Codex 外掛、Claude 市場、Vercel 技能、
# plain/Claude 技能和 MCP 設定。
...
# 新增產生的 Claude Code 市場，然後安裝其 Assets 外掛。
...
中繼資料； OAuth/設備設定發生在 MCP 主機中或透過應用程式的正常設定
設定流程。

Vercel Labs `skills` 轉接器是便攜式 `skills/<name>/SKILL.md` 捆綁包
適用於 `npx skills@latest add ...`，但原始 `skills` CLI 僅安裝說明。
它不執行儲存庫定義的安裝後指令碼或註冊 MCP 連線器。
...
標記，以便將來的狀態輸出可以識別來源和雜湊值。

產生的 Agent-Native 應用程式和工作區還包括框架提供的
`.agents/skills` 下的 skills（或 `packages/shared/.agents/skills` 中的
工作區）。使用以下指令從目前/最新的 CLI 重新整理那些支架 skills：
...
`AGENTS.md` 和 `.agents/skills` 保持規範。更新指令還可以修復
Claude 相容性連結（`CLAUDE.md` 和 `.claude/skills`），因此 Claude 程式碼可見
相同的說明，無需維護第二個副本。
