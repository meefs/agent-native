...
<Diagram id="doc-block-b44ilg" title={"段階的な開示"} summary={"すべてのスキルの名前と説明のみが常にコンテキストに含まれます。タスクが一致すると、ボディ全体がオンデマンドでロードされます。"}>

```html
<div class="sk-flow">
  <div class="diagram-card">
    <span class="diagram-pill accent">常にシステムプロンプト内</span>
    <div class="sk-list">
...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      lines: "3-5",
      label: "トリガー",
      note: "この `description` は、常にコンテキスト内にある**唯一**のテキストです。スキルが「いつ」適用されるかを正確に示すようにします。",
    },
    {
      lines: "9-14",
      label: "まずはルール",
    ...
    },
    {
      lines: "27-30",
      label: "クロスリンク",
      note: "関連するスキルをポイントすると、エージェントはガイダンスを再度導き出すのではなく、それらを連鎖させることができます。",
    },
  ]}
/>
...
<Diagram id="doc-block-amdqoz" title={"どのエージェントがどのスキルをロードするか"} summary={"スコープは、アプリ内ランタイム エージェントがスキルを認識するかどうかを決定します。dev スキルは、コーディング エージェントにのみ表示されます。"}>
...
# マーケットプレイス アダプターの構築: Codex プラグイン、Claude マーケットプレイス、Vercel スキル、
# plain/Claude スキルと MCP 構成。
npx @agent-native/core@latest app-skill pack --manifest templates/assets/agent-native.app-skill.json --out ./dist/assets-skill

# Vercel/open スキル CLI を使用してローカルにエクスポートされたバンドルをインストールします。
npx skills@latest add ./dist/assets-skill --skill assets -a codex -y

# 生成された Claude Code マーケットプレイスを追加し、その Assets プラグインをインストールします。
...
# または、ローカル パッケージ スクリプトに依存せずに、次のようにします。
