...
<Diagram id="doc-block-eiboxf" title={"प्रगतिशील खुलासा"} summary={"प्रत्येक कौशल का केवल नाम + विवरण हमेशा संदर्भ में होता है। जब कार्य मेल खाता है तो पूरा शरीर मांग पर लोड होता है।"}>

```html
<div class="sk-flow">
  <div class="diagram-card">
    <span class="diagram-pill accent">हमेशा system prompt में</span>
    <div class="sk-list">
  ...
  <div class="diagram-box">
    Full <code>SKILL.md</code> body<br /><small class="diagram-muted"
      >नियम, कोड, क्या करें/न करें</small
    >
  </div>
</div>
```
...
<AnnotatedCode
  id="doc-block-go2hpz"
  title={"SKILL.md की शारीरिक रचना"}
  ...
  annotations={[
    {
      lines: "2",
      label: "डिस्कवरी कुंजी",
      note: "The `name` matches the folder; it is how the skill is invoked as `/project-imports`.",
    },
    {
      lines: "3-5",
      label: "ट्रिगर",
      note: "यह `description` हमेशा संदर्भ में **एकमात्र** पाठ है। यह स्पष्ट रूप से बताएं कि *कब* कौशल लागू होता है।",
    },
    {
      lines: "9-14",
      label: "नियम पहले",
    ...
    },
    {
      lines: "27-30",
      label: "क्रॉस-लिंक",
      note: "संबंधित कौशलों पर ध्यान दें ताकि एजेंट मार्गदर्शन पुनः प्राप्त करने के बजाय उन्हें श्रृंखलाबद्ध कर सके।",
    },
  ]}
/>
...
<Diagram id="doc-block-kt8x06" title={"कौन सा एजेंट कौन सा कौशल लोड करता है"} summary={"स्कोप यह तय करता है कि इन-ऐप रनटाइम एजेंट किसी कौशल को देखता है या नहीं। देव कौशल केवल आपके कोडिंग एजेंट को दिखाई देते हैं।"}>
...
# Vercel/open Skills CLI: केवल निर्यात किए गए निर्देश, कोई MCP कॉन्फ़िगरेशन नहीं।
npx skills@latest add BuilderIO/agent-native --skill assets

# स्थानीय एजेंट क्लाइंट के लिए होस्टेड MCP कनेक्टर पंजीकृत करें।
npx @agent-native/core@latest app-skill ensure --manifest templates/assets/agent-native.app-skill.json

# संपादन योग्य स्थानीय स्रोत को मूर्त रूप दें और चलाएं।
...
# मार्केटप्लेस एडाप्टर बनाएं: Codex प्लगइन, Claude मार्केटप्लेस, Vercel कौशल,
# plain/Claude कौशल और MCP कॉन्फ़िगरेशन।
npx @agent-native/core@latest app-skill pack --manifest templates/assets/agent-native.app-skill.json --out ./dist/assets-skill

# Vercel/open Skills CLI के साथ स्थानीय निर्यातित बंडल इंस्टॉल करें।
npx skills@latest add ./dist/assets-skill --skill assets -a codex -y

# जेनरेट किए गए Claude कोड मार्केटप्लेस को जोड़ें, फिर इसके एसेट्स प्लगइन को इंस्टॉल करें।
...
# या, स्थानीय पैकेज स्क्रिप्ट पर भरोसा किए बिना:
